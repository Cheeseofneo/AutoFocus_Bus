...

{
    ...
    Angle=45;   // area with no scale number
    percent=0;  // default pos of indicator
}
...
void dial::paintEvent(QPaintEvent *event)
{
    ...
    painter.scale(side / 200.0, side / 200.0); //修改painter自己的坐标系
...
void dial::drawBg(QPainter *painter)
{
    int r = radius;
    painter->save();
...
//绘制刻度线
void dial::drawDial(QPainter *painter)
{
    int r = radius*0.85;
    double lineWidth = 1;
    painter->save();
...
//绘制动态速度值
void dial::drawText(QPainter *painter)
{
    painter->save();
    ...
    int w = (int)fm.horizontalAdvance(speed);
    int h = (int)fm.height();

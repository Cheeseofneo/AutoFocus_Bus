...
void dialog::InitialSetting()
{
    ...
    //填充串口数据位
    ui->comboBoxData->addItem("7 bits");
    ui->comboBoxData->addItem("8 bits");
    ui->comboBoxData->addItem("9 bits");
...
void dialog::connectconfig(){ // 设置父窗口的配置项

    if (!m_mainwindow->modbusDevice)
        return;

    m_mainwindow->modbusDevice->setConnectionParameter(QModbusDevice::SerialPortNameParameter,
                                                       ui->comboBoxPort->currentText());
    m_mainwindow->modbusDevice->setConnectionParameter(QModbusDevice::SerialBaudRateParameter,
    ...
    {
        case 0: m_mainwindow->modbusDevice->setConnectionParameter(QModbusDevice::SerialParityParameter,
    ...
    {
        case 0:m_mainwindow->modbusDevice->setConnectionParameter(QModbusDevice::SerialDataBitsParameter,
                                                              QSerialPort::Data8);break;
        default: break;
    }

    switch(ui->comboBoxStop->currentIndex())                     //设置停止位
    {
        case 1: m_mainwindow->modbusDevice->setConnectionParameter(QModbusDevice::SerialStopBitsParameter,
                                                                   QSerialPort::OneStop);break;
        case 2: m_mainwindow->modbusDevice->setConnectionParameter(QModbusDevice::SerialStopBitsParameter,
                                                                   QSerialPort::TwoStop);break;
        default: break;
    }
...
void dialog::connectclose(){  // 后续需要改成销毁对象

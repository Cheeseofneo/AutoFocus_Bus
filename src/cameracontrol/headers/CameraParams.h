...
typedef struct _MV_SAVE_IMAGE_PARAM_T_
{
    ...
    unsigned int       enPixelType;        // [IN]     输入数据的像素格式
    ...
    unsigned int    enImageType;        // [IN]     输出图片格式
...
typedef struct _MV_PIXEL_CONVERT_PARAM_T_
{
    ...
    unsigned int    enSrcPixelType;     // [IN]     源像素格式
    unsigned char*      pSrcData;           // [IN]     输入数据缓存
    unsigned int        nSrcDataLen;        // [IN]     输入数据大小

    unsigned int    enDstPixelType;     // [IN]     目标像素格式
...
{
    unsigned int    enType;                             // 节点类型
    unsigned int       enVisivility;                       //是否可见
...
{
    unsigned int   enAccessMode;       // 访问模式
}MV_XML_FEATURE_Base;
